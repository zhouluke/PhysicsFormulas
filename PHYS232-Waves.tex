...
\begin{equation*}
	\omega^2_\text{isothermal} \propto \kcol{p} \hhtab\hhtab
	\omega^2_\text{adiabatic} \propto \kcol{\gamma p}
\end{equation*}

\section{Oscillations Involving Massive Springs} \label{ch3:sec-massive-springs}
Suppose this time the spring itself has a mass of $M$. Suppose it has a spring contant $k$ and that there is a mass $m$ attached to it.

Each point on the spring undergoes a displacement proportional to its distance from the fixed end (just like in static extension).
