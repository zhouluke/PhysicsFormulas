...
Because we have two possibilities for $p$, our solution to \eqref{ch3:eq-xdd} could have been written more generally as\footnote{In general, when working with second-order differential equations like \eqref{ch3:eq-xdd}, we need to introduce two constants -- one for each level of integration. These constants could be $C_1, C_2$, or as we will see, $A$ and $\alpha$.}:
...
The solution to \eqref{ch3:eq-damping-NL} is:
\begin{equation}
\boxed{
...
\subsection{Solving for the equation of motion with damping}
\begin{proof}[Solving for the equation of motion]
...
$p$ cannot be a purely real number -- if that were the case, $jp\gamma$ would have nothing to cancel it. So, assume:
\begin{equation} 
...
\subsection{Remarks on the graph for damped free oscillations}
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{phys232/Ch3-damping.png} \caption{Rapidly damped harmonic oscillations.}\label{ch3:fig-damping}
\end{figure}

Our solution \eqref{ch3:eq-damping-solution} is plotted in Figure~\ref{ch3:fig-damping}. Notice that:
\begin{itemize}
