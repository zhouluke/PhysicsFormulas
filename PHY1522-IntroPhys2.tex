...
\begin{center}
     ...
     \small{\href{https://github.com/zhouluke/PhysicsFormulas}{Luke Zhou} $\cdot$ PHY 1522 $\cdot$ Hiver 2022}
\end{center}

\section{Cinématique ($a$ constant)}
\begin{multicols}{2}
\noindent
\begin{gather*}
v = v_{0} + a\Delta t \\
...
\quad \text{(charge ponctuelle)}
 \]
 \[ \uvec{r} = \pm \uvec{i} \cos\theta \pm \uvec{j} \sin\theta \]
\tableindent ($\uvec r$ pointe de la charge source $q$ à la charge cible $Q$) \\
\tableindent ($\kappa = 1$ dans le vide et dans les conducteurs)
\halfline
...
\vec{E} = \frac{\vec{F}}{Q} 
\qquad
\vec{E} = \frac{k}{\kappa} \frac{q}{r^2 }  \uvec{r} \enspace \text{(charge ponctuelle)}
%\qquad 
...
Champ électrique : cas particuliers
\[ E =  \frac{\lambda}{2\pi\kappa\epsilon_0  r} \quad\text{(fil infini)} \qquad
 E = \frac{\sigma}{2\kappa\epsilon_0} \quad\text{(plan infini)} \]
\[ E =  \frac{kh}{\kappa (R^2 + h^2)^{3/2}} \, Q_\text{tot}  
	\quad \text{(anneau)} \]
\[ E = \frac{2\pi\sigma  k}{\kappa}\left( 1- \frac{h}{\sqrt{R^2+h^2}} \right) 
	\quad \text{(disque)} \]
...
Énergie potentielle de deux charges ponctuelles [$\si{\joule}$]
...
\begin{center}
	\href{https://opentextbc.ca/universityphysicsv3openstax/chapter/thin-lenses/}{
...
\begin{tabular}{@{\tableindent}ll}
	surface convexe & $R_1>0, R_2<0$ \\
	surface concave & $R_1<0, R_2>0$ \\
\end{tabular}
\halfline
...
Diffraction à travers une \underline{fente double} : \underline{maximums} d'intensité et distance entre les fentes $d$ (expérience de Young)
\[ d\sin\theta = m\lambda 
\qquad d  \approx \frac{m\lambda L}{y}
\qquad (m=0,1,2,\dots) \]

Diffraction à travers une \underline{fente simple} : \underline{minimums} d'intensité et largeur de fente $a$
\[ a\sin\theta = m\lambda 
\qquad a  \approx \frac{m\lambda L}{y}
\qquad (m=1,2,3,\dots) \]

Réseau de diffraction : franges claires
\[ d\sin\theta = m\lambda
\qquad (m=0,1,2,3,\dots) \]

Approximations aux petits angles
\[ \sin\theta \approx \theta 
\qquad \tan\theta \approx \theta
\qquad \cos\theta \approx 1-\frac{\theta^2}{2} \approx 1 
\]

Intensité : fente double
\[ I \propto \frac{\sin^2\left( \frac{\pi d}{\lambda} \sin\theta \right)}{\left( \frac{\pi d}{\lambda} \sin\theta \right)^2} \]

Interféromètre de Michelson : différence de marche
\[ \delta = 2(L_1-L_2) \]
\begin{align*}
	\delta =  m\lambda \quad \text{(constructive)}  \quad
	&(m=0,1,2,\dots) \\
	\delta =  \frac{(2m+1)}{2} \lambda  \quad \text{(destructive)} \quad
	&(m=0,1,2,\dots)
\end{align*}

Polarisation de lumière : loi de Malus [$I$ en lux]
\begin{align*}
	 I = I_0 \cos^2\varphi \qquad &\text{(lumière déjà polarisée)}\\
	 I = \frac{1}{2} I_0 \qquad &\text{(lumière non polarisée)}
\end{align*}
...
\[ \int \frac{x\,dx}{(x^2 \pm a^2)^{1/2}} = \sqrt{x^2 \pm a^2} \quad
