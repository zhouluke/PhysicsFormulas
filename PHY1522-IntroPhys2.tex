...
\begin{tabular}{@{\tableindent}p{29mm}<{\raggedright}p{18mm}<{\raggedright}cl}
	système isolé & $Q=0, W=0$ &$\Longrightarrow$ & $\Delta U = 0$ \\
	processus cyclique & $\Delta U = 0$ &$\Longrightarrow$ & $Q=W$ \\
	processus isochore & $W=0$ &$\Longrightarrow$ & $\Delta U = Q$ \\
	processus adiabatique & $Q=0$ &$\Longrightarrow$ & $\Delta U = -W$ \\
...
Énergie interne de $N$ molécules ou $n$ moles d'un gaz parfait
\[ U = \frac{3}{2} NkT = \frac{3}{2} nRT \]

Chaleur spécifique molaire d'un gaz parfait
\[ C_V = \frac{3}{2} R 
\qquad
C_P = \frac{5}{2} R
\qquad 
