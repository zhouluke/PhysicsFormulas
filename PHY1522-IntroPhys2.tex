...
Lentilles minces : distance focale et puissance [D = \si{\metre^{-1}}]
...
\begin{tabular}{@{\tableindent}llll}
	image virtuelle et droite & $m>0$ \\
	image réelle et inversée & $m<0$ \\
\end{tabular}
\halfline
...
\begin{enumerate}
	\item Rayon parallèle à l'axe $\rightarrow$ lentille $\rightarrow$ réfraction vers l'un des $F$
	\item Rayon passant à travers le centre de la lentille $\rightarrow$ aucune déviation
	\item Rayon passant par l'un des $F$ $\rightarrow$ lentille $\rightarrow$ parallèle à l'axe
\end{enumerate}
...
\[ \Delta L = 2(L_1-L_2) \]
\begin{align*}
	\Delta L =  m\lambda \quad \text{(constructive)}  \qquad
	&(m=0,1,2,\dots) \\
	\Delta L =  \frac{(2m+1)}{2} \lambda  \quad \text{(destructive)} \qquad
	&(m=0,1,2,\dots)
\end{align*}

Polarisation de lumière : loi de Malus
\[ I = I_0 \cos^2\varphi \]
