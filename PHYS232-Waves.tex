...
Thus:
\begin{equation}
\omega^2 = \frac{\kcol{2g}}{\mcol{l}} \label{ch3:eq-utube-result}
\end{equation}
...
\subsection{Shear \& Shear Modulus}
...
Notice that \eqref{ch3:eq-torsional-F} for \textit{shear deformations} is analogous to \eqref{ch3:eq-Youngs-F} for \textit{longitudinal deformations}.
...
Twist one spindle through a small $\theta$; the end of each strip is moved transversely through a distance $r\theta$. The angle of shear is thus: \[ \alpha = \frac{r\theta}{l} \]

Substituting this result into \eqref{ch3:eq-torsional-F}, we find that the restoring force $\Delta F$ exerted by each strip tangentially to the disk is:
