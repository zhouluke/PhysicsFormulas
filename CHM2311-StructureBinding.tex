...
\begin{tabular}{@{\tableindent}llp{2.4cm}<{\raggedright}@{}}
...
\begin{tabular}{@{\tableindent}p{2.5cm}<{\raggedright}p{4.5cm}@{}}
...
\begin{itemize} 
\item Atomic radius: size of outermost orbital containing e-. Larger $n$ or higher $Z_\text{eff} \Longrightarrow$ lower radius.
...
Conversions
\[ \SI{1}{\metre} = 10^9 \, \si{\nano\metre} = 10^{12} \, \si{\pico\metre} \qquad
...
Resonance Theory: points to remember
\begin{itemize}
...
\begin{tabular}{@{\tableindent}cll@{}}
	...
	6 & Octahedral 
	& $sp^3d^2$ \\
	7 & Pentagonal Bipyramidal & $sp^3d^3$ \\ 
	8 & Square Antiprismatic & $sp^3d^4$ \\
\end{tabular}
\extraline
\renewcommand{\arraystretch}{1.4}
...
\begin{itemize}
	...
	\item Irreducible representation: a row in the character table
...
\begin{itemize}
	...
	\item Symmetry-adapted linear combinations of orbitals (SALCs)
\end{itemize}
...
To remember when drawing molecular orbital (MO) diagrams
\begin{itemize}
...
\begin{enumerate}
	\item Determine the point group of the molecule (If $C_{\infty h}$ or  $D_{\infty h}$, use $C_{2h}$ \& $D_{2h}$ instead)
	\item Determine the point group for each of the central atom's AOs -- consider the molecule's point group
	\item Find the point group for each of the GOs (SALCs) 
	\item Combine AOs of the central atom to the GOs having suitable symmetry, as long as $\Delta E \leq \SI{13}{\electronvolt}$
\end{enumerate}
...
\subsection*{Point Group Chart}
\begin{center}
	\href{https://chemistry.stackexchange.com/questions/68996/what-is-the-symmetry-of-the-cuboctahedron-fcc-metal}{
		\includegraphics[width=18cm]{./chm2311/point-group-chart.jpeg}
	}
\end{center}

For the group $S_n$, $n$ must be even; otherwise, $S_n = C_{nh}$.
\extraline

\subsection*{Unit Circle}

\begin{center}
	\href{https://www.fotolip.com/unit-circle-6524.html}{
		\includegraphics[width=12cm]{./chm2311/Unit-Circle.png}
	}
\end{center}
