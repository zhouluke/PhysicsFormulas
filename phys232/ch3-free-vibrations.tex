...
\section{Overview: Simple Harmonic Oscillators} \label{ch3:sec-simple-springs}
...
Rewrite \eqref{ch3:eq-NL} in terms of $\omega^2 = k/m$:
\begin{align}
m\ddot{x} + kx &= 0 \notag\\
\ddot{x} + \omega^2 x  &= 0 
\label{ch3:eq-xdd}
...
Because $x_1 = C_1e^{j\omega t}$ and $x_2 = C_2e^{-j\omega t}$ are both solutions to \eqref{ch3:eq-xdd}, their sum should be a solution too%
\footnote{In general, when working with second-order differential equations like \eqref{ch3:eq-xdd}, we need to introduce two constants -- one for each level of integration. These constants could be $C_1, C_2$, or $x_0, v_0$, or as we will see, $A$ and $\alpha$.}:
\begin{equation}
...
\begin{equation}
	x=\Re(z) \where z=e^{\omega t + \alpha}
\end{equation}
\end{proof}
...
\footnote{Or, by small angle approximations: $y=l-l\cos\theta = l(1-\cos\theta) \approx \theta^2/2$, but because $\theta\approx x/l$, $y \approx x^2/2l$.}
(see Figure~\ref{ch3:fig-pendulum}):
\begin{align}
l^2 &= x^2 + (l-y)^2  \notag \\	
